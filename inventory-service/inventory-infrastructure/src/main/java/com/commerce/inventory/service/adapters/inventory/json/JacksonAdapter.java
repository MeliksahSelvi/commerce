package com.commerce.inventory.service.adapters.inventory.json;
...
public class JacksonAdapter implements JsonPort {
    ...
    public <T extends Serializable> String convertDataToJson(T data) {
        try {
            ...
            throw new InventoryDomainException(String.format("Could not create %s object", data.getClass().getName()), e);
        }
    }
    ...
    public <T extends Serializable> T exractDataFromJson(String payload, Class<T> outputType) {
        try {
            ...
            throw new InventoryDomainException(String.format("Could not read %s object! %s", outputType.getName()), e);
        }
    }
}
