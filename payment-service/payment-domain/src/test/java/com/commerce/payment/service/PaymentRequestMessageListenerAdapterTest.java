...
class PaymentRequestMessageListenerAdapterTest {

    PaymentRequestMessageListenerAdapter paymentRequestMessageListenerAdapter;

    @BeforeEach
    void setUp() {
        paymentRequestMessageListenerAdapter = new PaymentRequestMessageListenerAdapter(new FakePaymentRequestListenerHelper());
    }
    ...
    void should_completePayment() {
        ...
        //when
        var failureMessages = paymentRequestMessageListenerAdapter.completePayment(paymentRequest);
    ...
    void should_completePayment_fail_when_account_not_found() {
        ...
        var exception = assertThrows(AccountNotFoundException.class,
                () -> paymentRequestMessageListenerAdapter.completePayment(paymentRequest));
    ...
    void should_cancelPayment() {
        ...
        //when
        var failureMessages = paymentRequestMessageListenerAdapter.cancelPayment(paymentRequest);
    ...
    void should_cancelPayment_fail_when_account_not_found() {
        ...
        //then
        var exception = assertThrows(AccountNotFoundException.class,
                () -> paymentRequestMessageListenerAdapter.cancelPayment(paymentRequest));
        assertTrue(exception.getMessage().contains("Could not find Account"));
    }
